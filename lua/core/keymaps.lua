...
map('n', '<C-a>', '<cmd>lua require("utils.telescope_home").find_files()<CR>', opts)

-- live grep in HOME with Ctrl+f
map('n', '<C-f>', '<cmd>lua require("utils.telescope_home").live_grep()<CR>', opts)
...
map('n', '<C-k>', '<cmd>tabnew<CR><cmd>lua require("core.autocmds").rename_current_tab()<CR>', opts)
